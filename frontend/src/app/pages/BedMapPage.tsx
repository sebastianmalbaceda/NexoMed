...
export default function BedMapPage() {
  ...
  const admitMutation = useMutation({
    mutationFn: async (payload: AdmitForm & { bedId: string }) => {
      const allergies = payload.allergiesText
        ? payload.allergiesText
            .split(",")
            .map((s) => s.trim())
            .filter(Boolean)
        : [];
  ...
  const searchDniMutation = useMutation({
    mutationFn: (dni: string) =>
      api.get<Patient>(`/patients/search?dni=${encodeURIComponent(dni)}`),
    onSuccess: (patient) => {
      setDniFound(patient);
      resetAdmit({
        dni: patient.dni ?? "",
  ...
  const dischargeMutation = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["beds"] });
      qc.invalidateQueries({ queryKey: ["patients"] });
      // Invalidate nurse-specific cached queries so discharged patients
      // disappear from the nurse's active patient list immediately
      qc.invalidateQueries({ queryKey: ["patients", "nurse"] });
      ...
      qc.setQueryData(["beds"], (old: Bed[] | undefined) =>
        old?.map((b) =>
          b.patient?.id === patientId
            ? {
                ...b,
                patient: { ...b.patient!, assignedNurseId: user?.id ?? null },
              }
            : b,
        ),
      );
      if (selectedBed?.patient?.id === patientId) {
        setSelectedBed((prev) =>
          prev?.patient
            ? {
                ...prev,
                patient: { ...prev.patient, assignedNurseId: user?.id ?? null },
              }
            : prev,
        );
      }
  ...
  const unassignNurseMutation = useMutation({
    ...
    onMutate: async (patientId: string) => {
      ...
      if (selectedBed?.patient?.id === patientId) {
        setSelectedBed((prev) =>
          prev?.patient
            ? { ...prev, patient: { ...prev.patient, assignedNurseId: null } }
            : prev,
        );
      }
    },
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            ...
            <p className="text-slate-500 text-sm font-medium">
              {occupiedCount}/24 camas ocupadas
            </p>
          </div>
        ...
        <div className="flex gap-1 mt-5 bg-white rounded-2xl border border-slate-200 p-1 w-fit shadow-sm">
          ...
          >
            <span className="flex items-center gap-2">
              ...
              <span
                className={`text-[10px] font-black px-1.5 rounded-full ${activeTab === "general" ? "bg-white/20 text-white" : "bg-slate-100 text-slate-600"}`}
              >
                {occupiedCount}
              </span>
            </span>
          ...
          {isNurse && (
            ...
            >
              <span className="flex items-center gap-2">
                ...
                <span
                  className={`text-[10px] font-black px-1.5 rounded-full ${activeTab === "my-patients" ? "bg-white/20 text-white" : "bg-blue-100 text-blue-700"}`}
                >
            ...
            {activeTab === "my-patients" ? (
              <>
                ...
                <p className="text-slate-300 text-xs mt-1">
                  Usa el mapa General y asígnate pacientes
                </p>
              </>
            ) : (
              <p className="text-slate-400 font-bold">
                    ...
                    >
                      <div className="flex justify-between items-start">
                        <div className="flex items-center gap-1.5">
                          <BedDouble
                            className={`w-3.5 h-3.5 ${isOccupied ? "text-slate-700" : "text-slate-300"}`}
                          />
                        ...
                        <div className="flex items-center gap-1">
                          {isMyPatient && (
                            <span className="text-[8px] font-black px-1.5 py-0.5 rounded bg-blue-500 text-white">
                              MÍO
                            </span>
                          )}
                          {isOccupied && (
                            <span
                              className={`text-[8px] font-black px-1.5 py-0.5 rounded text-white tracking-widest ${styles?.badge}`}
                            >
                              {styles?.text}
                            </span>
                          )}
                      ...
                      {isOccupied ? (
                        <div className="mt-2">
                          <div className="flex items-center gap-1.5">
                            ...
                            <div className="flex items-center gap-1 min-w-0">
                              ...
                              {(() => {
                                ...
                                return (
                                  <span
                                    className={`w-1.5 h-1.5 rounded-full shrink-0 ${dotColors[patient.status] ?? "bg-emerald-500"}`}
                                  />
                                );
                              })()}
                            </div>
                          ...
                          <div className="flex items-center gap-2 mt-1.5 pt-1.5 border-t border-slate-100">
                            {patient.allergies &&
                              getAllergiesCount(patient.allergies) > 0 && (
                                <span className="text-[8px] font-bold text-red-500">
                                  🚫 ALERGIA
                                </span>
                              )}
                            {isCritical && (
                              <span className="text-[8px] animate-bounce">
                                ⚠️
                              </span>
                            )}
                          </div>
            ...
            <div className="flex-1 overflow-y-auto p-8">
              {selectedBed.patient ? (
                <div className="space-y-6">
                  ...
                  {/* Allergies */}
                  <div
                    className={`p-4 rounded-2xl border ${getAllergiesCount(selectedBed.patient.allergies) > 0 ? "bg-red-50 border-red-100" : "bg-emerald-50 border-emerald-100"}`}
                  >
                    <p
                      className={`text-[10px] font-black uppercase mb-1 ${getAllergiesCount(selectedBed.patient.allergies) > 0 ? "text-red-700" : "text-emerald-700"}`}
                    >
                      🚫 Alergias
                    </p>
                    <p
                      className={`text-xs font-bold ${getAllergiesCount(selectedBed.patient.allergies) > 0 ? "text-red-900" : "text-emerald-900"}`}
                    >
                      {parseAllergies(selectedBed.patient.allergies).join(
                        ", ",
                  ...
                  <div className="pt-4 border-t border-slate-100 space-y-3">
                    ...
                    >
                      <ExternalLink className="w-4 h-4" /> Ver ficha del
                      paciente
                    </button>
                    ...
                    {isNurse &&
                      (selectedBed.patient.assignedNurseId === user?.id ? (
                        <button
                          onClick={() =>
                            unassignNurseMutation.mutate(
                              selectedBed.patient!.id,
                            )
                          }
                        ...
                        <button
                          onClick={() =>
                            assignNurseMutation.mutate(selectedBed.patient!.id)
                          }
                        ...
                        >
                          <UserCheck className="w-4 h-4" /> Asignarme este
                          paciente
                        </button>
                      ))}
                    ...
                    {isDoctor && (
                      <button
                        onClick={() =>
                          navigate("/doctor", {
                            state: { patientId: selectedBed.patient!.id },
                          })
                        }
                ...
                <div className="py-6">
                  <div className="text-center mb-6">
                    ...
                    <p className="text-xs text-slate-400 mt-2">
                      Esta cama está lista para un nuevo ingreso.
                    </p>
                  </div>
                  ...
                  {canAdmit && showAdmitForm && (
                    <form
                      onSubmit={handleSubmitAdmit((data) => {
                        if (selectedBed)
                    ...
                    >
                      ...
                      {searchDniMutation.isError && (
                        <p className="text-xs text-slate-400">
                          Paciente no encontrado. Puedes registrar uno nuevo con
                          este DNI.
                        </p>
                      )}
                      {dniFound && (
                        <p className="text-xs text-emerald-600 font-medium">
                          ✓ Paciente encontrado. Datos auto-rellenados.
                        </p>
                      )}
                      ...
                      <div>
                        ...
                        <input
                          ...
                          {...registerAdmit("surnames")}
                          readOnly={!!dniFound}
                          className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 ring-slate-300 disabled:opacity-60"
                        />
                        {admitErrors.surnames && (
                          <p className="text-xs text-red-500 mt-1">
                            {admitErrors.surnames.message}
                          </p>
                        )}
                      </div>
                      ...
                      <div>
                        ...
                        <input
                          ...
                          {...registerAdmit("diagnosis")}
                          className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 ring-slate-300"
                        />
                        {admitErrors.diagnosis && (
                          <p className="text-xs text-red-500 mt-1">
                            {admitErrors.diagnosis.message}
                          </p>
                        )}
                      </div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {freeBeds.map((bed) => {
                      ...
                      return (
                        ...
                        >
                          <div
                            className={`w-8 h-8 rounded-lg flex items-center justify-center ${isSel ? "bg-blue-500 text-white" : "bg-slate-100 text-slate-400"}`}
                          >
                            <BedDouble className="w-4 h-4" />
                          </div>
                          <p className="text-xs font-bold text-slate-800">
                            Hab. {bed.room} - Cama {bed.letter}
                          </p>
                          {isSel && (
