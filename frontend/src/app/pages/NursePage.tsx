...
export default function NursePage() {
  ...
  const dedupedPatients = useMemo(() => {
    ...
    return patients.filter((p) => {
      const fullName = `${p.name} ${p.surnames ?? ""}`
        .trim()
        .replace(/\s+/g, " ");
  ...
  const scheduleUpdateMutation = useMutation({
    mutationFn: ({
      ...
      return api.put(`/medications/${medId}/schedule`, {
  ...
  function getSchedulesInWindow(
    med: Medication,
    win: { start: Date; end: Date },
  ) {
    const inWindow = (med.schedules || []).filter((s) => {
  ...
  const pendingMeds = medications.reduce((acc, med) => {
    const all = Object.values(shiftWindows).flatMap((win) =>
      getSchedulesInWindow(med, win),
    );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div
          className={`rounded-2xl p-5 text-white shadow-lg ${medNotifications.length > 0 ? "bg-amber-500 shadow-amber-100" : "bg-slate-500 shadow-slate-100"}`}
        >
          <p className="text-white/70 text-xs font-bold uppercase tracking-wide mb-1">
            Alertas medicación
          </p>
          ...
          {isLoading ? (
            ...
            <ul className="divide-y divide-slate-100">
              {dedupedPatients.map((p) => {
                ...
                const pNotifs = notifications.filter(
                  (n) => !n.read && n.relatedPatientId === p.id,
                ).length;
                    ...
                    >
                      <div className="flex items-start justify-between gap-2">
                        ...
                        <div className="flex flex-col items-end gap-1 shrink-0">
                          ...
                          {pNotifs > 0 && (
                            <span className="text-[10px] bg-amber-500 text-white font-black px-1.5 py-0.5 rounded">
                              🔔 {pNotifs}
                            </span>
                          )}
                        </div>
                ...
                <div className="bg-amber-50 border-l-4 border-amber-400 rounded-2xl px-5 py-4 flex items-start gap-3 shadow-sm shadow-amber-100">
                  ...
                  <div>
                    <p className="text-sm font-black text-amber-900">
                      {medNotifications.length === 1
                        ? "⚠️ El médico ha modificado la medicación"
                        : `⚠️ ${medNotifications.length} cambios en medicación`}
                    </p>
                    <ul className="mt-1 space-y-0.5">
                      {medNotifications.slice(0, 3).map((n) => (
                        ...
                        >
                          · {NOTIFICATION_TYPE_LABELS[n.type]}: {n.message}
                        </li>
                      ))}
                    </ul>
                  </div>
                    ...
                    <div className="bg-red-50 border border-red-100 rounded-xl p-3">
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {parseAllergies(selected.allergies).map((a) => (
                          <span
                            key={a}
                            className="text-xs bg-red-500 text-white font-bold px-2 py-0.5 rounded-full"
                          >
                            {a}
                          </span>
                        ))}
              ...
              <div className="bg-white border border-slate-200 border-t-4 border-t-orange-400 rounded-2xl overflow-hidden shadow-sm">
                ...
                {loadingMeds ? (
                  ...
                  <div className="divide-y divide-slate-100">
                    {medications.map((m) => {
                      const allMedSchedules = Object.values(
                        shiftWindows,
                      ).flatMap((win) => getSchedulesInWindow(m, win));
                      const allDone =
                        allMedSchedules.length > 0 &&
                        allMedSchedules.every((s) => !!s.administeredAt);
                      const someDone = allMedSchedules.some(
                        (s) => !!s.administeredAt,
                      );
                          ...
                          <div className="flex items-start justify-between gap-2 mb-3">
                            <div>
                              <div className="flex items-center gap-2 flex-wrap">
                                ...
                                <span
                                  className={`text-[10px] font-black px-2 py-0.5 rounded text-white ${allDone ? "bg-emerald-500" : someDone ? "bg-amber-500" : "bg-red-500"}`}
                                >
                                  {allDone
                                    ? "COMPLETADO"
                                    : someDone
                            ...
                            <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 mb-3">
                              ...
                              <p className="text-xs text-amber-700 font-medium mb-2">
                                ...
                                {Array.from(
                                  {
                                    length: Math.ceil(
                                      (24 - editStartHour) / m.frequencyHrs,
                                    ),
                                  },
                          ...
                          {(() => {
                            ...
                            return (
                              <div className={`grid ${cols} gap-2`}>
                                {SHIFTS.map(
                                  ({ key, label, color, bg, border }) => {
                                    const shiftSchedules = getSchedulesInWindow(
                                      m,
                                      shiftWindows[key],
                                    );
                                    ...
                                    return (
                                      <div
                                        key={key}
                                        className={`rounded-xl border p-2 ${isActive ? `${bg} ${border}` : "bg-slate-50 border-slate-100 opacity-60"}`}
                                      >
                                        <p
                                          className={`text-[10px] font-black uppercase tracking-wide mb-1.5 ${isActive ? color : "text-slate-400"}`}
                                        >
                                          {label}
                                        </p>
                    ...
                    <ul className="space-y-2 max-h-56 overflow-y-auto pr-1">
                      {careRecords.map((r) => {
                        ...
                        return (
                          <li
                            key={r.id}
                            className="flex items-start justify-between gap-3 bg-slate-50 rounded-xl px-3 py-2 border border-slate-100"
                          >
                            <div className="min-w-0">
                              <div className="flex items-center gap-2">
                                <span
                                  className={`text-[10px] font-black px-2 py-0.5 rounded-full border ${CARE_COLORS[r.type] ?? "bg-slate-100 text-slate-600 border-slate-200"}`}
                                >
                                  {r.type.toUpperCase()}
                                </span>
